...
function manvote()
{
    ...
    firebase.database().ref('/mastervalue/value').ref.once('value', function (snapshot) {
        var nvl = snapshot.val();
        if (nvl != null)
        {
        console.log(" nvl is : " + nvl);

        database.ref("/voted_ids/"+nvl+"/id").set(nvl);
        console.log("updated " + nvl + " to voted IDs");
        }
    ...
    firebase.database().ref('/mastervalue/value').ref.once('value', function (snapshot) {
        var chke = snapshot.val();
       console.log("idcheck val : " + chke);
